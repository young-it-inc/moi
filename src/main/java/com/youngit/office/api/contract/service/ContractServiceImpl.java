...
public class ContractServiceImpl implements ContractService {
    ...
    @Autowired
    public ContractServiceImpl(ContractMapper contractMapper, ContractMapstruct contractMapstruct) {
    ...
    public ContractDto getOneContract(String contractUniqNo) {
        ...
        List<ContractDetailModel> contractDetailModelList = contractMapper.getOneContractDetailList(contractUniqNo);
        List<ContractProductModel> contractProductModelList = contractMapper.getOneContractProductList(contractUniqNo);

        ContractDto result = contractMapstruct.toDto(contractModel);
        List<ContractDetailDto> detailResultList = contractDetailModelList.stream().map(contractMapstruct::toDetailDto).toList();
        List<ContractProductDto> productResultList = contractProductModelList.stream().map(contractMapstruct::toProductDto).toList();
        result.setContractDetailList(detailResultList);
        result.setContractProductList(productResultList);
        return result;
    }

    @Override
    public List<ContractDetailDto> getOneContractDetailList(String contractUniqNo) {
        List<ContractDetailModel> contractDetailModelList = contractMapper.getOneContractDetailList(contractUniqNo);
        return contractDetailModelList.stream().map(contractMapstruct::toDetailDto).toList();
    }

    @Override
    public List<ContractProductDto> getOneContractProductList(String contractUniqNo) {
        List<ContractProductModel> contractProductModelList = contractMapper.getOneContractProductList(contractUniqNo);
        return contractProductModelList.stream().map(contractMapstruct::toProductDto).toList();
    }
