    ...
    public byte[] generateExcelAsList(List<AsDto> asDtoList) throws IOException {
        ...
        try(Workbook workbook = new XSSFWorkbook())
        {
            ...
            String[] headers = {"경과", "상태", "배정번호", "관리사업소", "접수일", "A/S횟수", "수용가번호", "수용가성명", "수용가주소", "수용가연락처",
                    "수용가연락처2", "검침원명", "검침원연락처", "A/S전영상", "A/S후영상", "구경", "A/S처리일", "A/S결과", "A/S원인코드", "단가",
            ...
            for (int i = 0; i < asModelList.size(); i++) {
                ...
                row.createCell(15).setCellValue(asModel.getEstimateAmount()); //구경
                row.createCell(16).setCellValue(asModel.getEstimateAmount()); //A/S처리일
                row.createCell(17).setCellValue(asModel.getEstimateAmount()); //A/S결과
                row.createCell(18).setCellValue(asModel.getEstimateAmount()); //A/S원인코드
                row.createCell(19).setCellValue(asModel.getEstimateAmount()); //단가

                row.createCell(20).setCellValue(asModel.getEstimateAmount()); //A/S서브코드
                row.createCell(21).setCellValue(asModel.getEstimateAmount()); //A/S직원
                row.createCell(22).setCellValue(asModel.getEstimateAmount()); //설치길이
                row.createCell(23).setCellValue(asModel.getEstimateAmount()); //A/S접수사유
                row.createCell(24).setCellValue(asModel.getEstimateAmount()); //A/S메모
                row.createCell(25).setCellValue(asModel.getEstimateAmount()); //A/S특이사항
                row.createCell(26).setCellValue(asModel.getEstimateAmount()); //검침기위치
                row.createCell(27).setCellValue(asModel.getEstimateAmount()); //단자함위치
                row.createCell(28).setCellValue(asModel.getEstimateAmount()); //설치일
                row.createCell(29).setCellValue(asModel.getEstimateAmount()); //설치자

                row.createCell(30).setCellValue(asModel.getEstimateAmount()); //제품구분
                row.createCell(31).setCellValue(asModel.getEstimateAmount()); //검침기일련번호
                row.createCell(32).setCellValue(asModel.getEstimateAmount()); //단자함일련번호
                row.createCell(33).setCellValue(asModel.getEstimateAmount()); //외부표시기
                row.createCell(34).setCellValue(asModel.getEstimateAmount()); //외함설치
                row.createCell(35).setCellValue(asModel.getEstimateAmount()); //설치환경
                row.createCell(36).setCellValue(asModel.getEstimateAmount()); //통신선상태
                row.createCell(37).setCellValue(asModel.getEstimateAmount()); //거래처고유ID
                row.createCell(38).setCellValue(asModel.getEstimateAmount()); //A/S고유ID
                row.createCell(39).setCellValue(asModel.getEstimateAmount()); //시공사고유ID

                row.createCell(40).setCellValue(asModel.getEstimateAmount()); //설치고유ID
*/
                // 데이터 행 가운데 정렬 스타일 적용
