...
public class AsModel {
    ...
    String	asCauseCode; //A/S원인코드(48개):
    // 1001(경과제품신규설치)/1002(습기)/1003(무반응)/1004(무지개영상-4p제품)/1005(실리콘(제품오염))/1006(포커스불량-10p제품)/1007(제품내부기포발생-10p제품)/1008(계량기교체)/1009(임의탈착)/1010(수용가공사)
    // 1011(설치불량)/1012(파손)/1013(분실)/1014(프로그램오류)/1015(수용가번호오류)/1016(철거)/1017(이전설치)/1018(정상)/1019(계량기문제/불량)/1020(부재)
    // 1021(적재/주차)/1022(수용가거부)/1023(오류접수)/1024(미방문)/1025(단자접촉불량)/1026(통신에러)/1027(케이블단선)/1028(전월처리완료)/1029(영상흐림지침확인불가)/1030(내구연한경과(미처리))
    // 1031(내구연한경과(현장점검))/1032(지침영상잘림)/1033(단자위치변경)/1034(LED불량)/1035(노후단자교체)/
    // 1036(선택x?)/1037/1038/1039(선택x?)/1040(선택x?)
    // 2001(외표기-연결된장치없음)/2002(외표기-하얀화면)/2003(외표기-화면깜빡임)/2004(외표기-)/2005(외표기-)/2006(외표기-)
    // 7777(신규설치)/ 9999(이월처리예정)
